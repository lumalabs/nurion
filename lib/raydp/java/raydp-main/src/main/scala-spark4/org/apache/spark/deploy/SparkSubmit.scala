...
private[spark] class SparkSubmit extends Logging {
  ...
  private[deploy] def prepareSubmitEnvironment(
    ...
    val clusterManager: Int = args.maybeMaster match {
      case Some(v) =>
        ...
        v match {
          ...
          // RayDP patch: fall back to OTHERS instead of erroring, so custom
          // cluster managers (e.g. Ray) can plug in via a custom master URL.
          case _ => OTHERS
        }
    ...
    val options = List[OptionAssigner](
      ...
      // RayDP patch: include OTHERS so jars are forwarded to custom cluster managers.
      OptionAssigner(args.jars, STANDALONE | KUBERNETES | OTHERS, ALL_DEPLOY_MODES,
        confKey = JARS.key),
...
object SparkSubmit extends CommandLineUtils with Logging {
  ...
  // RayDP patch: OTHERS represents custom cluster managers such as Ray.
  private val OTHERS = 32
  private val ALL_CLUSTER_MGRS = YARN | STANDALONE | LOCAL | KUBERNETES | OTHERS
