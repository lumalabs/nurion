...
object ObjectStoreWriter {
  ...
  def fromSparkRDD(df: DataFrame, storageLevel: StorageLevel): Array[Array[Byte]] = {
    ...
    val queue = ObjectRefHolder.getQueue(uuid)
    // Spark 4 patch: `toArrowBatchRdd` and `sqlContext` are package-private on
    // the classic Dataset (Dataset became an abstract trait for Spark Connect).
    val classicDf = df.asInstanceOf[org.apache.spark.sql.classic.Dataset[_]]
    val rdd = classicDf.toArrowBatchRdd
    rdd.persist(storageLevel)
    rdd.count()
    var executorIds = classicDf.sqlContext.sparkContext.getExecutorIds.toArray
...
//    }
    ...
    val handles = executorIds.map {id =>
      Ray.getActor(s"raydp-executor-${df.sparkSession.sparkContext.appName}-$id")
         .get
