...
# =============================================================================
# 4. Partition Management Tests (4 tests)
# =============================================================================


class TestPartitionManagement(StabilityTestBase):
    """Tests for multi-partition queue management."""

    @pytest.mark.asyncio
    async def test_multi_partition_parallel_consumption(self, ray_cluster):
        """Multiple partitions consumed in parallel.

        Scenario: 4 workers consuming from 4 partitions.
        Expected: All data processed, no duplicates.
        """
        NUM_RECORDS = 1200
        validator = DataValidator()
        source_data = generate_test_data_with_checksum(NUM_RECORDS)
    ...
    @pytest.mark.asyncio
    async def test_partition_rebalance_during_scaling(self, ray_cluster):
        """Partition rebalance when workers scale.

        Scenario: Workers added/removed, partitions should rebalance.
        Expected: All partitions consumed, no data loss.
        """
        NUM_RECORDS = 1000
        validator = DataValidator()
        source_data = generate_test_data_with_checksum(NUM_RECORDS)
        ...
        try:
            ...
            # Trigger rebalance by killing a worker
    ...
    @pytest.mark.asyncio
    async def test_partition_assignment_correctness(self, ray_cluster):
        """Partition assignment distributes work correctly.

        Scenario: Multiple workers should share partitions fairly.
        Expected: All data processed without gaps.
        """
        NUM_RECORDS = 800
        EXPLODE_FACTOR = 2
        validator = DataValidator()
    ...
    @pytest.mark.asyncio
    async def test_no_partition_starvation(self, ray_cluster):
        """No partition left unconsumed.

        Scenario: More partitions than workers.
        Expected: All partitions eventually consumed.
        """
        NUM_RECORDS = 600
        validator = DataValidator()
        source_data = generate_test_data_with_checksum(NUM_RECORDS)
        ...
        # All records should be present (no starved partitions)
        assert validator.verify_count(sink_data, NUM_RECORDS)
        # Verify all IDs present
        actual_ids = {r["id"] for r in sink_data}
        expected_ids = set(range(NUM_RECORDS))
        missing = expected_ids - actual_ids
        assert not missing, f"Missing {len(missing)} IDs - partition starvation"


# =============================================================================
# 5. Backpressure Tests (3 tests)
# =============================================================================
