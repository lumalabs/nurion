...
class SolsticePortal:
    ...
    def _create_app(self) -> FastAPI:
        ...
        async def portal_home(request: Request):
            ...
            # Get running jobs
            running_jobs = []
            jobs_dict = ray.get(self.registry.list_jobs.remote(), timeout=2)
        ...
        async def running_jobs_page(request: Request):
            ...
            return self.templates.TemplateResponse("running_jobs.html", {
                "request": request,
                "jobs": running_jobs,
            })
        ...
        async def completed_jobs_page(request: Request):
            ...
            return self.templates.TemplateResponse("completed_jobs.html", {
                "request": request,
                "jobs": completed_jobs,
            })
        ...
        async def job_detail_page(job_id: str, request: Request):
            ...
            job_info = ray.get(self.registry.get_job.remote(job_id), timeout=1)
            if job_info:
                # Running job - render template
                # TODO: Fetch stage details from RayJobRunner
                return self.templates.TemplateResponse("job_detail.html", {
                    "request": request,
                    "job": job_info,
                    "stages": [],  # TODO: Get from runner
        ...
        async def stage_detail_page(job_id: str, stage_id: str, request: Request):
            """Stage detail page."""
            # Check if job is running
            # In embedded mode, we can fetch real-time data
            # But here we are in the Portal (separate process)
            # We need to query the JobWebUI actor via Ray
            
            # For now, let's use the API approach: fetch from local storage if completed,
            # or try to get from JobWebUI if running.
            
            stage_data = None
            
            # 1. Try running job
            try:
                job_reg = ray.get(self.registry.get_job.remote(job_id), timeout=1)
                if job_reg and job_reg.runner_actor_name:
                    # TODO: Call JobWebUI actor to get stage details
                    pass
            except Exception:
                pass
            ...
            if stage_data:
                return self.templates.TemplateResponse("stage_detail.html", {
                    "request": request,
                    "job_id": job_id,
                    "stage": stage_data,
                })
            
            # Fallback: Render basic template that fetches data via HTMX
            return self.templates.TemplateResponse("stage_detail.html", {
                "request": request,
                "job_id": job_id,
                "stage": {"stage_id": stage_id, "operator_type": "Loading..."},
            })
        ...
        async def worker_detail_page(job_id: str, worker_id: str, request: Request):
            ...
            return self.templates.TemplateResponse("worker_detail.html", {
                "request": request,
                "job_id": job_id,
                "worker": worker_data,
            })
        ...
        async def exceptions_page(job_id: str, request: Request):
            ...
            return self.templates.TemplateResponse("exceptions.html", {
                "request": request,
                "job_id": job_id,
        ...
        async def checkpoints_page(job_id: str, request: Request):
            """Checkpoints page."""
            return self.templates.TemplateResponse("checkpoints.html", {
                "request": request,
                "job_id": job_id,
            })
        ...
        async def lineage_page(job_id: str, request: Request):
            """Lineage page."""
            return self.templates.TemplateResponse("lineage.html", {
                "request": request,
                "job_id": job_id,
            })
