...
#### 1. Queue Backend Infrastructure (`runtime/queue/`)
...
#### 2. Stage Master (`runtime/core/stage_master.py`)
...
#### 3. Split Payload Store (`runtime/core/split_payload_store.py`)
...
#### 4. Simplified Stage API (`runtime/core/stage.py`)

- Stage only accepts `OperatorConfig` dataclass
- Removed complex master_config hierarchy
- Cleaner API

#### 5. Runner (`runtime/runtime/ray_runner.py`)
